...
At the extreme, every single effect (besides running forever or crashing) carried by the program is reified that way. We then speak about *an effect system*. Effect systems can be made generic and reusable. This is what pure functional languages usually do to support IO actions:
...
const run = async (effect) => {
  if (effect.type === "writeFile") {
    ...
    return await readFile(effect.path, effect.encoding);
  } else if (effect.type === "concurrent") {
...
const main = bindEffect(
  setTimeoutEffect(
    () => logEffect("this should not be printed"),
    2000,
  ),
...
export const instrumentImpure = async (url, content) => {
  ...
  while (true) {
    const urls = extractMissingUrlArrayPure(url, files);
    if (urls.length === 0) {
      return instrumentPure(url, files);
    }
